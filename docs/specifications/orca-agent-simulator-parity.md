...
- upstream の kd-tree 実装自体は未対応で、近傍探索は `NaiveNeighborSearch`
- open chain semantics は研究室拡張であり、upstream の closed polygon 前提と完全一致ではない
...
次段階の主要 gap は regression support の強化である。
