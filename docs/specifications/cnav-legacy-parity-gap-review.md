...
現行 validation scenario は分岐確認には十分だが、`forPaper.cpp` や `crowdForPaper.cpp` の direct parity とは言えない。  
後続で legacy parity をより強く主張するには、baseline driver の構造をもう 1 段近づけた scenario または code-generated setup が必要である。

#### 2. trace comparison strength

Python 側 trace は整ったが、C++ 側 trace export が未整備なので、まだ「どの step で何が違うか」を数値で言い切れない。  
この差分は tooling gap であり、後続作業で埋める価値が高い。
...
- C++ 側 trace exporter
- `forPaper` 系 scenario の direct port
