...
| temporary goal model | `forPaper` 系は temporary goal を計算して preferred velocity を更新 | issue `0035` で `navigation_grid` + `goal_sequence` により再現導線を追加 | exact timing 以外の obstacle / cell field 差分を縮められる | scenario guidance gap | high |
...
そのため、`2026-03-14` 付で direct-port scenario を追加し、さらに issue `0035` で `forPaper` 側の `changeDest()` と temporary goal guidance を Python へ切り出した。  
現在の主要 gap は scenario setup そのものより、timing semantics と legacy decision model に寄っている。
...
issue `0035` ではこれを `navigation_grid` と `goal_sequence` として切り出し、mainline CNav 本体から分離したまま Python 側へ持ち込んだ。  
今後は timing / stop 条件まで legacy に寄せるかどうかを別論点として扱える。
...
- C++ 側 trace exporter
- `forPaper` 系 guidance の timing / finalize semantics 整理
