...
- `update_every_step`
- `minimum_action_update_step`
- `apply_selected_action_next_step`
...
- `apply_selected_action_next_step = false`
...
`external/CNav_MyStyle` の driver 固定値を code 上で引ける helper として、`create_mystyle_driver_settings("forpaper")` と `create_mystyle_driver_settings("crowd-forpaper")` も追加されている。  
これらは `time_step`、`AgentProfile`、legacy decision profile を束ねて返す。
...
- `minimum_action_update_step > 0` のとき、その step までは初期 intended velocity を保持する
- `apply_selected_action_next_step = true` のとき、更新 step で選ばれた action は次 step から適用する
