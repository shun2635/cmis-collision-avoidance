...
- `src/cmis_ca/algorithms/cnav/algorithm.py` に `latest_step_trace` を追加し、step ごとの action evaluation と出力速度を保持するようにした
- `src/cmis_ca/algorithms/cnav/trace.py` を追加し、trace model、scenario runner、JSONL writer を実装した
- `scripts/dump_cnav_trace.py` を追加し、validation scenario の trace を JSONL で吐けるようにした
- `tests/algorithms/test_cnav_trace.py` を追加し、validation scenario を使った trace smoke を入れた
- trace の列と利用方法を `docs/specifications/cnav-trace-parity-harness.md` に固定した
