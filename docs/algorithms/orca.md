...
また、ORCA 側の agent-agent / obstacle 制約生成も `algorithms/orca/constraints.py` に実装済みである。  
一方で、`ORCAAlgorithm.step()` 側では obstacle line の protected constraint 数を solver へまだ渡していないため、upstream の `linearProgram3` 相当をフルに活かす統合は次段階に残っている。
