...
float simTimeStep = 0.1f; // タイムステップの長さ
float ooparts = 0.4f; //謎のパラメータ。使わないけど除去が面倒だから残っている。
const int numNeighbors = 4;//考慮する近傍エージェントの数, エージェント全体の数より多いとバグる。要改良。
static const int lengthSimulateTimeSteps = 5; //予測タイムステップ数
...
void evaluateEachAction(RVO::RVOSimulator* sim, std::ofstream &outputFile, std::vector<bool> isInGoal,
						float coordFactor, bool allNeigh, int contadourX, int numActions, float ActionDirection[9], bool followNeighbors, std::vector<RVO::Vector2> temporaryGoals)  /* timeHorizonで定義された将来のタイムステップ数にわたり、各action/velocityについてシミュレーションする */
{	
	sim->buildTree();
					...
					float coordFactor, int numActions) //Initialize the Simulation, positions of the obstacles and the agents
{
	...
	obstacle3.push_back(RVO::Vector2(-widthOfWall/2, thicknessOfHall/2));
	obstacle3.push_back(RVO::Vector2(-widthOfWall/2, -thicknessOfHall/2));
	obstacle3.push_back(RVO::Vector2(-widthOfHall/2, -thicknessOfHall/2));
	obstacle3.push_back(RVO::Vector2(-widthOfHall/2, thicknessOfHall/2));

	obstacle4.push_back(RVO::Vector2(widthOfHall/2, thicknessOfHall/2));
	obstacle4.push_back(RVO::Vector2(widthOfHall/2, -thicknessOfHall/2));
	obstacle4.push_back(RVO::Vector2(widthOfWall/2, -thicknessOfHall/2));
	obstacle4.push_back(RVO::Vector2(widthOfWall/2, thicknessOfHall/2));
	...
	/* 上側 */
	for (int i = 0; i < 5; i++){
		sim->addAgent(RVO::Vector2(0.0f, 50.0f + i * radiusORCA * 2.2f));
		finalGoals.push_back(RVO::Vector2(0.0f, -50.0f));
		sim->setAgentGoal(i, RVO::Vector2(0.0f, -50.0f));
	}
	...
	for (int i = 0; i < 5; i++){
		sim->addAgent(RVO::Vector2(0.0f,  -50.0f - i * radiusORCA * 2.2f));
		finalGoals.push_back(RVO::Vector2(0.0f, 50.0f));
		sim->setAgentGoal(i+5, RVO::Vector2(0.0f, 50.0f));
	}
	...
	for (size_t i = 0; i < sim->getNumAgents(); ++i) {
		...
		if (goalIsPositiveSide){
			if ((!positionIsPositiveSide) && (!nearO)){
				...
				thisTemporaryGoal = sim->getAgentGoal(i);
			}
		}else{
			if ((positionIsPositiveSide) && (!nearO)){
				thisTemporaryGoal = RVO::Vector2(0.0f, 5.0f);
...
int main()
{
	...
	std::string positionFilePath = "/Users/tamurashuntarou/Downloads/CML/results/CNav_MyStyle/compromise/position.csv";
	...
	do{
		...
		if((algorithm == 2) && (timeStep > 1)){
			evaluateEachAction(sim, actionFile, isInGoal, coordFactor, allNeigh, contadourX, numActions, ActionDirection, followNeighbors, temporaryGoals); 
