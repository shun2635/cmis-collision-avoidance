...
{	
	...
	for (int myID = 0; myID < static_cast<int>(sim->getNumAgents()); ++myID){
		...
		if (!isInGoal[myID]){					
            ...
            RVO::Vector2 myTemporaryGoalVector = sim->getAgentMaxSpeed(myID) * RVO::normalize(temporaryGoals[myID] - sim->getAgentPosition(myID));
			float angle =  ActionDirection[finalAction];
			...
			sim->setAgentPosition(myID,RVO::Vector2(-1000.0f,-1000.0f));
			sim->setAgentVelocity(myID, RVO::Vector2(0.0f, 0.0f));							 
		}
    }
}
					...
					float coordFactor, int numActions, int numNeighbors) //Initialize the Simulation, positions of the obstacles and the agents
{
	...
	for (int i = 0; i < 5; i++){
		...
		finalGoals.push_back(RVO::Vector2(0.0f, -100.0f));
		sim->setAgentGoal(i, RVO::Vector2(0.0f, -100.0f));
	}
	...
	for (int i = 0; i < 5; i++){
		sim->addAgent(RVO::Vector2(0.0f,  -50.0f - i * radiusORCA * 2.2f));
		finalGoals.push_back(RVO::Vector2(0.0f, 100.0f));
		sim->setAgentGoal(i+5, RVO::Vector2(0.0f, 100.0f));
	}
			...
			float sameWeight)
{
	...
	float velMag[9]= {1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 0.1};
