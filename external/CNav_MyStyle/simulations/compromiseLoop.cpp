					...
					float coordFactor, int numActions, int numNeighbors) //Initialize the Simulation, positions of the obstacles and the agents
{
	...
	*/
	...
	/* 上側 */
	for (int i = 0; i < 1; i++){
		sim->addAgent(RVO::Vector2(0.0f, 100.0f + i * radiusORCA * 2.2f));
		finalGoals.push_back(RVO::Vector2(0.0f, -200.0f));
		sim->setAgentGoal(i, RVO::Vector2(0.0f, -200.0f));
	}
	...
	for (int i = 0; i < 1; i++){
		sim->addAgent(RVO::Vector2(0.0f,  -100.0f - i * radiusORCA * 2.2f));
		finalGoals.push_back(RVO::Vector2(0.0f, 200.0f));
		sim->setAgentGoal(i+1, RVO::Vector2(0.0f, 200.0f));
	}
	...
	for (int thisIter = 0; thisIter < iterNum; thisIter++){
		...
		*/
		...
		do{
			...
			//std::cout<< "agent 1 chose "<< chosenAction[1] <<std::endl;
