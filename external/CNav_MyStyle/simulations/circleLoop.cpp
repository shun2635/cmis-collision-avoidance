...
static const int lengthSimulateTimeSteps = 10; //予測タイムステップ数
...
void mainLoop(int iterNum, float coordFactor)
{
    /* -------------Setting----------------- */
	...
	for (int thisIterNum = 0; thisIterNum < iterNum; thisIterNum++){
		/* 終了キーをfalseにセット */
		bool finalize = false;
		/* シミュレーションを作成 */
		...
		std::string positionFilePath = "/Users/tamurashuntarou/Downloads/CML/results/CNav/circle/position_coord-"+std::to_string(coordFactor).substr(0, 4)+"_iter-"+std::to_string(thisIterNum)+".csv";
		...
		*/
		...
		std::cout << coordFactor << "," << thisIterNum << "," << sim->getGlobalTime() << std::endl;
